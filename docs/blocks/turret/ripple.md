# <img id="spr" src="/wiki/images/block-ripple-large.png"></img> Ripple

*"An extremely powerful artillery turret. Shoots clusters of shells at enemies over long distances."*


|General||
| --- | --- |
|Internal Name|`ripple`|
|Solid|Yes|
|Health|1170    |
|Size|3x3  |
|Build Time|4.5  seconds  |
|Build Cost|<a href="/wiki/items/copper"><img id="spr" src="/wiki/images/item-copper-xlarge.png"/></a>x150 <a href="/wiki/items/graphite"><img id="spr" src="/wiki/images/item-graphite-xlarge.png"/></a>x135 <a href="/wiki/items/titanium"><img id="spr" src="/wiki/images/item-titanium-xlarge.png"/></a>x60  |

|Liquids||
| --- | --- |
|Liquid Capacity|20  liquid units  |

|Shooting||
| --- | --- |
|Range|36.25  blocks  |
|Inaccuracy|12  degrees  |
|Shots|4    |
|Shots/Second|1    |
|Targets Air|No  |
|Targets Ground|Yes  |
|Ammo|![graphite1](../../images/graphite1.png) Graphite <br> • **33** area dmg ~** 3.1** tiles <br> • **2**x ammo multiplier <br> • **0.8** knockback <br>![silicon1](../../images/silicon1.png) Silicon <br> • **33** area dmg ~** 3.1** tiles <br> • **2**x ammo multiplier <br> • **0.8** knockback <br> • **homing** <br>![pyratite1](../../images/pyratite1.png) Pyratite <br> • **30** area dmg ~** 3.1** tiles <br> • **2**x ammo multiplier <br> • **0.8** knockback <br> • **incendiary** <br>![blast-compound1](../../images/blast-compound1.png) Blast Compound <br> • **50** area dmg ~** 5.6** tiles <br> • **4**x ammo multiplier <br> • **0.8** knockback <br>![plastanium1](../../images/plastanium1.png) Plastanium <br> • **45** area dmg ~** 4.4** tiles <br> • **2**x ammo multiplier <br> • **1.0** knockback <br> • **frag** <br> |

|Optional Enhancements||
| --- | --- |
|Booster|<a href="/wiki/liquids/water"><img id="spr" src="/wiki/images/liquid-water.png"/></a>12/sec / <a href="/wiki/liquids/cryofluid"><img id="spr" src="/wiki/images/liquid-cryofluid.png"/></a>12/sec  ![water1](../../images/water1.png) Water <br> • **1.4**x fire rate ![cryofluid1](../../images/cryofluid1.png) Cryofluid <br> • **1.9**x fire rate <br> |


--- 
[comment]: # (WARNING: Do not modify the text above. It is automatically generated every release.)
# <img id="spr" src="../../../images/spectre-icon-large.png"></img> Spectre 

<a href="../../../items/copper.md"><img id="spr" src="../../../images/item-copper-medium.png"/></a>x700 <a href="../../../items/graphite.md"><img id="spr" src="../../../images/item-graphite-medium.png"/></a>x600 <a href="../../../items/thorium.md"><img id="spr" src="../../../images/item-thorium-medium.png"/></a>x500 <a href="../../../items/plastanium.md"><img id="spr" src="../../../images/item-plastanium-medium.png"/></a>x350 <a href="../../../items/surge-alloy.md"><img id="spr" src="../../../images/item-surge-alloy-medium.png"/></a>x500 

*"A large turret which shoots two powerful bullets at once."*


|General||
| --- | --- |
|Internal Name|`spectre`|
|Solid|Yes|
|Health|2480    |
|Size|4x4  |
|Build Time|40.92  seconds  |

|Liquids||
| --- | --- |
|Liquid Capacity|20  liquid units  |

|Input/Output||
| --- | --- |
|Input|![liquid-water](../../images/liquid-water.png) 120 /sec Water / ![liquid-cryofluid](../../images/liquid-cryofluid.png) 120 /sec Cryofluid  |

|Shooting||
| --- | --- |
|Range|25  blocks  |
|Inaccuracy|3  degrees  |
|Shots|2    |
|Shots/Second|10    |
|Targets Air|Yes  |
|Targets Ground|Yes  |
|Ammo|![item-graphite-medium](../../images/item-graphite-medium.png) Graphite <br> • **42** damage <br>![item-pyratite-medium](../../images/item-pyratite-medium.png) Pyratite <br> • **38** damage <br> • **incendiary** <br>![item-thorium-medium](../../images/item-thorium-medium.png) Thorium <br> • **65** damage <br> |

|Optional Enhancements||
| --- | --- |
|Boost Effect|2 x speed  |


--- 
[comment]: # (WARNING: Do not modify the text above. It is automatically generated every release.)

















...
| `stack` | A container that layers its children on top of each other (all children share the same cell). No node-specific properties.                                            | `stack{ image{ region: "ok" size: 300 } label: "Overlaid text" }` |
